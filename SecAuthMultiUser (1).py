...
twofa = random.randint(1362, 326151) #generate before hand beacause its cleaner looking.
sectwofa = twofa
...
# ban system used to be here. ban detection would be at top, running beofore all main code. for imediate banning
 ...
 #   with open("BannedUsers.txt", "a") as bannedg:
 #       bannedg.write(f"{usrname}\n")
if usrname.lower() in ["dev", "su", "root", "admin"]:
    console = True
    while console == True:
        command = input("Dev console. use help for commands.")
        pass
        if command == "help":
            print("Commands: whoami, stop, WipeUsrData ")
        pass
        if command == "whoami":
            print("You are: ",usrname)
        pass
        if command == "users":
            print(accounts) #for debug purposes only! (just to confirm user data is getting loaded properly!)
        pass
        if command == "stop":
            console == False
            print("Console session has ended.")
        pass
        if command == "WipeUsrData":
            LastChance = False
            Warn = input("Are you sure? this wipes ALL user data!")
            if Warn.lower() == "yes":
                LastChance == True
                if LastChance == True:
                    with open("UserAccountData.txt", "w") as Delete:
                        Delete.write(f"")
            else:
                pass
